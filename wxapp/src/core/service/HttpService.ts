...
import { UserMpLoginRequest, UserMpLoginResponse } from '../../protocol/UserProto';
...
export function formatUrl(url: string): string {
    return !url.startsWith('https://') && !url.startsWith('http://') ? environment.host + url : url;
}
...
export const httpService = new class {
    ...
    request(message: WebpbMessage): Observable<any> {
        ...
        if (!this.loginSubject) {
            ...
            from(RxWx.login())
                .pipe(
                    mergeMap(res => this.doRequest<UserMpLoginResponse>(UserMpLoginRequest.create({
                        appId: Const.APP_ID,
                        code: res.code
                    }))),
    ...
    private doRequest = <T>(message: WebpbMessage): Observable<T> => {
        ...
        return new Observable<WechatMiniprogram.RequestSuccessCallbackResult>(subscriber => {
            ...
            catchError(_ => of({ statusCode: -1, data: { message: '请求失败' } })),
            mergeMap(res => {
